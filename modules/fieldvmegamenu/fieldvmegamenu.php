...
class fieldvmegamenu extends Module {
					...
					<div id="main_image_upload" data-type-content="image">
						...
						<a href="#" class="removemainimg '.(isset($link_params["main_image"]) && $link_params["main_image"] != "" ? "" : " hide").'">Remove</a>
                                ...
                                <div class="margin-form activeLabel">
                                        ...
                                        <p>Icon example: <strong>icon-home</strong><br/> You can see http://fortawesome.github.io/Font-Awesome/3.2.1/cheatsheet/ for complete list of available icons.</p>
                                </div>
	...
	private function flexMenu()
	{
		...
		$isr++;
			...
			$this->_menu .= '<li class="v-megamenuitem id_menu'.$link['id_field_vmegamenu_menus'].(($submenu) ? ' hasChildren' : '').(($link['narrow'] == 1)  ? ' narrow' : '').(($this->curPath() == $itemData['link']) ? ' current-item' : '').(($isr>7 && $kt>7 ) ? ' more_here" style="display:none;"' : '"').' data-menuid="'.$link['id_field_vmegamenu_menus'].'"><a class="menu-title" href="'.$itemData['link'].'" '.(((!empty($itemData['new_window'])) && ($itemData['new_window'] == 1)) ? ' target="_blank"' : '' ).'><i class="'.(($link['main_icon'] != "")  ? $link['main_icon'] : '').'"></i><span class="item-icon title_font">'.$itemData['name'].'</span></a>'.(($submenu) ? '<a href="#" class="opener"></a>' : '').'';
			if ($submenu) {
			...
			$this->_menu .= '</li>'.PHP_EOL;
			if($isr==$kt && $isr > 7){ $this->_menu .="<li  class='more-vmegamenu'><a><span>" .$this->l('All Categories'). "<i class='icon-angle-double-right'></i></span></a></li>";  }
		}
	}
